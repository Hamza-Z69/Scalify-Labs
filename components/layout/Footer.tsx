import Link from "next/link";
import { ScalifyLogo } from "@/components/ScalifyLogo";

const servicesLinks = [
  { label: "Recrutement de Franchisés", href: "/services/recrutement-franchises" },
  { label: "Trafic Local", href: "/services/trafic-local" },
  { label: "Accompagnement Digital", href: "/services/accompagnement-digital" },
  { label: "IA & Performance", href: "/services/ia-performance" },
];

const agencyLinks = [
  { label: "À propos", href: "/a-propos" },
  { label: "Cas clients", href: "/cas-clients" },
  { label: "Blog", href: "/blog" },
  { label: "Franchise Maroc", href: "/franchise-maroc" },
];

const legalLinks = [
  { label: "Mentions légales", href: "/mentions-legales" },
  { label: "Politique de confidentialité", href: "/politique-confidentialite" },
];

export function Footer() {
  return (
    <footer
      style={{
        background: "rgba(232,134,42,0.02)",
        borderTop: "1px solid rgba(232,134,42,0.06)",
      }}
      aria-label="Pied de page"
    >
      {/* Scoped hover styles for server component */}
      <style
        dangerouslySetInnerHTML={{
          __html: `
            .footer-link {
              color: rgba(240,236,229,0.55);
              transition: color 0.2s cubic-bezier(.22,1,.36,1);
            }
            .footer-link:hover {
              color: #E8862A;
            }
            .footer-social {
              color: rgba(240,236,229,0.55);
              transition: color 0.2s cubic-bezier(.22,1,.36,1);
            }
            .footer-social:hover {
              color: #E8862A;
            }
          `,
        }}
      />

      <div className="max-w-7xl mx-auto px-6 py-16 md:py-20">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
          {/* Brand */}
          <div className="lg:col-span-1">
            <Link href="/" aria-label="Scalify Labs — Accueil">
              <ScalifyLogo className="h-9 w-auto" />
            </Link>
            <p
              className="mt-4 text-[14px] leading-relaxed"
              style={{
                fontFamily: "var(--font-dm-sans), sans-serif",
                color: "rgba(240,236,229,0.55)",
              }}
            >
              Franchise Growth Agency
            </p>
          </div>

          {/* Services */}
          <div>
            <h4
              className="mb-5 text-[11px] font-semibold uppercase"
              style={{
                fontFamily: "var(--font-dm-sans), sans-serif",
                letterSpacing: "0.1em",
                color: "rgba(240,236,229,0.25)",
              }}
            >
              Services
            </h4>
            <ul className="flex flex-col gap-3">
              {servicesLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="footer-link text-[13px] inline-block"
                    style={{ fontFamily: "var(--font-dm-sans), sans-serif" }}
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* L'Agence */}
          <div>
            <h4
              className="mb-5 text-[11px] font-semibold uppercase"
              style={{
                fontFamily: "var(--font-dm-sans), sans-serif",
                letterSpacing: "0.1em",
                color: "rgba(240,236,229,0.25)",
              }}
            >
              L&apos;Agence
            </h4>
            <ul className="flex flex-col gap-3">
              {agencyLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="footer-link text-[13px] inline-block"
                    style={{ fontFamily: "var(--font-dm-sans), sans-serif" }}
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h4
              className="mb-5 text-[11px] font-semibold uppercase"
              style={{
                fontFamily: "var(--font-dm-sans), sans-serif",
                letterSpacing: "0.1em",
                color: "rgba(240,236,229,0.25)",
              }}
            >
              Contact
            </h4>
            <ul
              className="flex flex-col gap-3 text-[13px]"
              style={{
                fontFamily: "var(--font-dm-sans), sans-serif",
                color: "rgba(240,236,229,0.55)",
              }}
            >
              <li>
                <a
                  href="mailto:hello@scalify.ma"
                  className="footer-link inline-block"
                >
                  hello@scalify.ma
                </a>
              </li>
              <li>
                <a
                  href="tel:+2126XXXXXXXX"
                  className="footer-link inline-block"
                >
                  +212 6XX XXX XXX
                </a>
              </li>
              <li>Casablanca, Maroc</li>
            </ul>

            {/* Social icons */}
            <div className="flex items-center gap-4 mt-6">
              <a
                href="https://www.linkedin.com/company/scalify-labs"
                target="_blank"
                rel="noopener noreferrer"
                className="footer-social"
                aria-label="LinkedIn"
              >
                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                </svg>
              </a>
              <a
                href="https://www.instagram.com/scalifylabs"
                target="_blank"
                rel="noopener noreferrer"
                className="footer-social"
                aria-label="Instagram"
              >
                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z" />
                </svg>
              </a>
            </div>
          </div>
        </div>
      </div>

      {/* Bottom bar */}
      <div style={{ borderTop: "1px solid rgba(232,134,42,0.06)" }}>
        <div className="max-w-7xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4 py-5">
          <p
            className="text-[13px]"
            style={{
              fontFamily: "var(--font-dm-sans), sans-serif",
              color: "rgba(240,236,229,0.55)",
            }}
          >
            &copy; {new Date().getFullYear()} Scalify Labs. Tous droits réservés.
          </p>
          <nav aria-label="Liens légaux">
            <ul className="flex items-center gap-6">
              {legalLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="footer-link text-[13px] inline-block"
                    style={{ fontFamily: "var(--font-dm-sans), sans-serif" }}
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </nav>
        </div>
      </div>
    </footer>
  );
}
